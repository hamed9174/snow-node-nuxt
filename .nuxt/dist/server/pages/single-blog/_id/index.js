...
//
//
/* harmony default export */ var _idvue_type_script_lang_js_ = ({name:"SingleBlog",head:{title:'Single Blog'},data(){return{webUrl: xhr["b" /* webUrl */],formTitle:'',method:'',addReplyComment:false,comment:'',replyComment:'',orgComment:{},comment_id:'',comments:[],blog:null,id:null,index:null};},async asyncData({$api,params}){const blog=await $api.$get(`blogs/single-blog/${params.id}`);return{blog:blog};},methods:{EditComment(index){this.addReplyComment=!this.addReplyComment;this.index=index;this.replyComment=this.blog.comments[index].message;this.method='edit';this.formTitle='Edit Comment';},async AddComment(){let comment={message:this.comment,writer:this.$auth.user.username,replies:[]};this.blog.comments.push(comment);await this.$api.put(`blogs/comment`,this.blog).then(res=>{this.blog=res.data;}).catch(err=>console.log(err));this.comment='';this.$refs.add.reset();},ShowReplyComment(index){this.index=index;this.addReplyComment=!this.addReplyComment;this.method='reply';this.formTitle='Reply Comment';this.replyComment='';},async ChangeComment(){if(this.method==='reply'){let reply={message:this.replyComment,writer:this.$auth.user.username};this.blog.comments[this.index].replies.push(reply);await this.$api.put(`blogs/comment`,this.blog).then(res=>{this.blog=res.data;}).catch(err=>console.log(err));}else{this.blog.comments[this.index].message=this.replyComment;await this.$api.put(`blogs/comment`,this.blog).then(res=>{this.blog=res.data;}).catch(err=>console.log(err));}this.replyComment='';this.addReplyComment=!this.addReplyComment;this.$refs.reply.reset();},async DeleteComment(index){this.replyComment=this.blog.comments[index].message;let comments=this.blog.comments.filter(comment=>comment.message!==this.replyComment);this.blog.comments=comments;await this.$api.put(`blogs/comment`,this.blog).then(res=>{this.blog=res.data;}).catch(err=>console.log(err));}}});
