...
//
//
/* harmony default export */ var portfoliovue_type_script_lang_js_ = ({name:"Portfolios",head:{title:'Portfolio'},components:{Portfolio: Portfolio["default"]},data(){return{menuActive:false,portfolio:[],portfolios:[]};},async asyncData({$api}){const portfolios=await $api.$get('portfolios');return{portfolios:portfolios};},methods:{MenuActive(event,portfolios){event.target.parentNode.getElementsByTagName('li').forEach(item=>{item.classList.remove('active');});event.target.classList.add('active');this.PortfoliosUpdate(portfolios);},PortfoliosUpdate(portfolios){if(portfolios==='all'){this.portfolios=this.portfolio;}else{let portfolioCategory=this.portfolio.filter(item=>item.category==portfolios);this.portfolios=portfolioCategory;}}}});
