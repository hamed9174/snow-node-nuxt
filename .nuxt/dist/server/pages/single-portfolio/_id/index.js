...
//
//
/* harmony default export */ var _idvue_type_script_lang_js_ = ({name:"SinglePortfolio",head:{title:'Single Portfolio'},data(){return{webUrl: xhr["b" /* webUrl */],formTitle:'',method:'',addReplyComment:false,comment:'',replyComment:'',orgComment:{},comments:[],portfolio:null,id:null,index:null};},async asyncData({$api,params}){const portfolio=await $api.$get(`portfolios/single-portfolio/${params.id}`);return{portfolio:portfolio};},watch:{addReplyComment:function(val){if(!val){this.replyComment='';}}},methods:{EditComment(index){this.addReplyComment=!this.addReplyComment;this.index=index;this.replyComment=this.portfolio.comments[index].message;this.method='edit';this.formTitle='Edit Comment';},async AddComment(){let comment={message:this.comment,writer:this.$auth.user.username,replies:[]};this.portfolio.comments.push(comment);await this.$api.put(`portfolios/comment`,this.portfolio).then(res=>{this.portfolio=res.data;}).catch(err=>console.log(err));this.comment='';this.$refs.add.reset();},ShowReplyComment(index){this.index=index;this.addReplyComment=!this.addReplyComment;this.method='reply';this.formTitle='Reply Comment';},async ChangeComment(){if(this.method==='reply'){let reply={message:this.replyComment,writer:this.$auth.user.username};this.portfolio.comments[this.index].replies.push(reply);await this.$api.put(`portfolios/comment`,this.portfolio).then(res=>{this.portfolio=res.data;}).catch(err=>console.log(err));}else{this.portfolio.comments[this.index].message=this.replyComment;await this.$api.put(`portfolios/comment`,this.portfolio).then(res=>{this.portfolio=res.data;}).catch(err=>console.log(err));}this.replyComment='';this.addReplyComment=!this.addReplyComment;this.$refs.reply.reset();},async DeleteComment(index){this.replyComment=this.portfolio.comments[index].message;let comments=this.portfolio.comments.filter(comment=>comment.message!==this.replyComment);this.portfolio.comments=comments;await this.$api.put(`portfolios/comment`,this.portfolio).then(res=>{this.portfolio=res.data;}).catch(err=>console.log(err));}}});
